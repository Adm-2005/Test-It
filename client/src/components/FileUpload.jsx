...
export default function FileUpload({ images, setImages }) {
    ...
    return (
        ...
        >
            ...
            {
                images.length > 0 && (
                    <div className={images.length > 2 ? 'm-1 grid grid-rows-2 grid-cols-2 overflow-hidden gap-1' : 'm-1 mx-auto grid grid-rows-1 grid-cols-2 overflow-hidden gap-1'}>
                        {
                            images.map((image, index) => (
                                <div className='flex justify-between items-center gap-1 bg-slate-400 p-[2px] rounded-md' key={index}>
                                    <div className='flex h-full relative bottom-[2px] items-center'>
                                        <p className='text-sm text-white overflow-hidden whitespace-nowrap max-w-[90px]'>{image.name}</p>
                                    </div>
                                    <IconContext.Provider value={{ size: '15px', color: '#ffffff' }}>
                                        <MdClear onClick={() => handleImageRemoval(index)}/>
                                    </IconContext.Provider>
                                </div>
                            ))
                        }
...

            {
                images.length > 0 && (
                    <span className='flex justify-center items-center gap-1 mt-2'>
                        ...
                        <p className='text-slate-600 text-sm'>{images.length}/4 image(s) selected</p>
                    </span>
                )
            }
        </div>
